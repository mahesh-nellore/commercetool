...
public class Product extends BaseTest{
	...
	public Map<String, String> getProductDetails(String baseUri, String path, String token, String projectKey, String productId) {
		RestAssured.baseURI = baseUri;
		Map<String, String> map = new HashMap<String, String>();
		
		try {
			HttpGet request = new HttpGet("https://api.australia-southeast1.gcp.commercetools.com/"+projectKey+"/product-projections/key="+productId);
			request.addHeader("Authorization", "Bearer " + token);
			CloseableHttpClient httpClient = HttpClients.createDefault();
			CloseableHttpResponse response = httpClient.execute(request);
			int statusCode = response.getStatusLine().getStatusCode();
			if(200 == statusCode) {
				logger.log(Status.PASS, "Get the Product Details API calls is Success and the status code is: "+statusCode);
				...
				logger.log(Status.FAIL, "Get the product details API call is Failed and the status code is: "+statusCode);
			}
